...
// 定数を保持するストレージ
struct KeyStorage {
    static let networkStatusChanged = "networkStatusChanged" // ネットワーク状態が変更されたことを示すキー
}
struct NetworkErrorMessages {
    static let networkConnectionError = "ネットワークに接続できません。"
}
// ネットワークの状態を表す列挙型
enum NetworkState: String {
    case reachable = "20000"   // ネットワークが正常な状態
    case unreachable = "40000" // ネットワークに問題がある状態
}
...
final class NetworkUtil{
    ...
    @objc public func checkNetworkStatus() {
        ...
        if isReachable && !needsConnection {
            _ = operateStoredData(action: "save", storedValues: NetworkState.reachable.rawValue, forKey: KeyStorage.networkStatusChanged)
            setNotificationCenter()
            stopTimer()
        } else {
            _ = operateStoredData(action: "save", storedValues: NetworkState.unreachable.rawValue, forKey: KeyStorage.networkStatusChanged)
            setNotificationCenter()
            startTimer()
        }
    }
